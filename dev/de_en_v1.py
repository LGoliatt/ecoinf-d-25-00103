...
for run in range(0, n_runs):
    ...
    for dataset in datasets:
        ...
        for tk, tn in enumerate(dataset['target_names']):
            ...
            #print(s)
            #------------------------------------------------------------------
            lb  = [0.0]*n_features  + [-10]*n_features  + [ 1e-4,   -10, ]
            ub  = [1.0]*n_features  + [+10]*n_features  + [ 1e+0,   +10, ]
            #------------------------------------------------------------------ 
            ...
            for strategy in strategy_list[:1]:
                ...
                def objective_function(x,*args):
                    ...
                    x[n_features:2*n_features] = np.where(np.abs(x[n_features:2*n_features])<0.001,0,x[n_features:2*n_features])
                    a=np.array(x[n_features:2*n_features])
                    b=x[-1]                    
                    k=0
                    if sum(ft)==0:
                        return 1e12
                    
                    r = (sum((np.sum(X[:,ft]*a[ft], axis=1)+ b - y)**2)/n_samples)**0.5
                    r+=x[-2]*np.linalg.norm(a[ft],1)
                    return r
                ...
                res = de(objective_function, tuple(zip(lb,ub)), args=args,
                             strategy=strategy,
                             init=init, maxiter=400, tol=1e-5,  
